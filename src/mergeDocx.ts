...
const mergeParsedXML = (
  templateParsedXML: any, // eslint-disable-line @typescript-eslint/no-explicit-any
  contentParsedXMLBody: any, // eslint-disable-line @typescript-eslint/no-explicit-any
  {
    pattern,
    insertStart,
    insertEnd,
          ...
          'w:body': bodyContent.reduce(
            (acc, bodyItem) => {
              if (bodyItem['w:sectPr']) {
                return [...acc, ...(insertEnd ? contentParsedXMLBody : []), bodyItem];
              }
...
export const mergeDocx = (
  ...
  const finalParsedXML = mergeParsedXML(templateParsedXML, contentParsedXMLBody, { pattern, insertStart, insertEnd });
  ...
  zip.getEntries().forEach((zipEntry) => {
    const entryBuffer = zipEntry.entryName === documentXMLPath ? Buffer.from(finalXML, 'utf8') : zipEntry.getData();
