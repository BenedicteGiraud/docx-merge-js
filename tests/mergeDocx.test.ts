...
describe('mergeDocx', () => {
  ...
  it('throws error if source or content path is missing', () => {
    expect(() => mergeDocx('', './tests/fixtures/table.docx', {})).toThrow('Missing source or content path');
    expect(() => mergeDocx('./tests/fixtures/template.docx', '', {})).toThrow('Missing source or content path');
  });

  it('throws error if insert position or pattern is missing', () => {
    expect(() => mergeDocx('./tests/fixtures/template.docx', './tests/fixtures/table.docx', {})).toThrow(
      "At least one of 'pattern', 'insertStart', or 'insertEnd' must be provided.",
    );
  });

  it('throws error if the path are not valid', () => {
    expect(() =>
      mergeDocx('./tests/fixtures/nonexistent.docx', './tests/fixtures/table.docx', { pattern: '{{table}}' }),
    ).toThrow('Source file does not exist: ./tests/fixtures/nonexistent.docx');

    expect(() =>
      mergeDocx('./tests/fixtures/template.docx', './tests/fixtures/nonexistent.docx', { pattern: '{{table}}' }),
    ).toThrow('Content file does not exist: ./tests/fixtures/nonexistent.docx');
  });

  it('throws error if the document are not docx files', () => {
    expect(() =>
      mergeDocx('./tests/fixtures/template.pdf', './tests/fixtures/table.docx', { pattern: '{{table}}' }),
    ).toThrow('Invalid file extension. Only .docx files are supported');

    expect(() =>
      mergeDocx('./tests/fixtures/template.docx', './tests/fixtures/template.pdf', { pattern: '{{table}}' }),
    ).toThrow('Invalid file extension. Only .docx files are supported');
  });
});
